...
class ShootGenerator
{
  public:
    ...
    struct Shot
    {
         ...
         */
        Shot(const rcsc::Vector2D &point, const double &speed, const rcsc::AngleDeg &angle)
            : point_(point), vel_(rcsc::Vector2D::polar2vector(speed, angle)), speed_(speed), angle_(angle),
              goalie_never_reach_(true), score_(0)
        {
        }
    };
     ...
     */
    struct ScoreCmp : public std::binary_function<Shot, Shot, bool>
    {
        /*!
  ...
  public:
     ...
     */
    const ShotCont &getShots(const rcsc::PlayerAgent *agent)
    {
        search(agent);
        return M_shots;
    }
  ...
  private:
    ...
    void calculateShotPoint(const rcsc::WorldModel &wm, const rcsc::Vector2D &shot_point,
                            const rcsc::PlayerObject *goalie);
    bool canScore(const rcsc::WorldModel &wm, const bool one_step_kick, Shot *shot);
    bool maybeGoalieCatch(const rcsc::WorldModel &wm, const rcsc::PlayerObject *goalie, Shot *shot);

    int predictOpponentReachStep(const rcsc::WorldModel &wm, const rcsc::Vector2D &target_point,
                                 const rcsc::PlayerObject *opponent, const rcsc::Vector2D &first_ball_pos,
                                 const rcsc::Vector2D &first_ball_vel, const bool one_step_kick, const int max_step);
};
