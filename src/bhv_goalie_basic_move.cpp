...
bool Bhv_GoalieBasicMove::execute(PlayerAgent *agent)
{
    ...
    dlog.addText(Logger::TEAM, __FILE__ ": Bhv_GoalieBasicMove. move_point(%.2f %.2f)", move_point.x, move_point.y);

    //////////////////////////////////////////////////////////////////
    // tackle
    if (Bhv_BasicTackle(0.8, 90.0).execute(agent))
    {
        dlog.addText(Logger::TEAM, __FILE__ ": tackle");
        return true;
    }
    ...
    if (!Body_GoToPoint2010(move_point, 1.0, ServerParam::i().maxDashPower()).execute(agent))
...
Vector2D Bhv_GoalieBasicMove::getTargetPoint(PlayerAgent *agent)
{
    ...
    {
        ball_reach_step =
            std::min(wm.interceptTable()->teammateReachCycle(), wm.interceptTable()->opponentReachCycle());
    }
    const Vector2D base_pos = wm.ball().inertiaPoint(ball_reach_step);

    return GoalieFormation().getPosition(base_pos, wm);

    //---------------------------------------------------------//
    // angle is very dangerous
    if (base_pos.y > ServerParam::i().goalHalfWidth() + 3.0)
    {
        Vector2D right_pole(-ServerParam::i().pitchHalfLength(), ServerParam::i().goalHalfWidth());
    ...
    //---------------------------------------------------------//
    {
        ...
        const Vector2D base_point(-ServerParam::i().pitchHalfLength() - x_back, 0.0);
        Vector2D ball_point;
        ...
        else
        {
            ...
            if (opp_min < ball_pred_cycle)
            {
                ...
                dlog.addText(Logger::TEAM, __FILE__ ": opp may reach near future. cycle = %d", opp_min);
            }

            ball_point = inertia_n_step_point(base_pos, wm.ball().vel(), ball_pred_cycle, ServerParam::i().ballDecay());
