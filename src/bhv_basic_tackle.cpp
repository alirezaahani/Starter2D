 ...
 */
bool Bhv_BasicTackle::execute(PlayerAgent *agent)
{
    ...
    if (wm.self().card() == NO_CARD && tackle_prob < wm.self().foulProbability())
    {
    ...
    {
        dlog.addText(Logger::TEAM, __FILE__ ": failed. low tackle_prob=%.2f < %.2f", wm.self().tackleProbability(),
                     M_min_probability);
        return false;
    }
    ...
    if (self_reach_point.x < -SP.pitchHalfLength())
    {
        ...
        const Line2D goal_line(Vector2D(-SP.pitchHalfLength(), 10.0), Vector2D(-SP.pitchHalfLength(), -10.0));
        ...
        if (intersect.isValid() && intersect.absY() < SP.goalHalfWidth() + 1.0)
        {
            ...
            dlog.addText(Logger::TEAM, __FILE__ ": ball will be in our goal. intersect=(%.2f %.2f)", intersect.x,
                         intersect.y);
        }
    }

    if (wm.existKickableOpponent() || ball_will_be_in_our_goal || (opp_min < self_min - 3 && opp_min < mate_min - 3) ||
        (self_min >= 5 && wm.ball().pos().dist2(SP.theirTeamGoalPos()) < std::pow(10.0, 2) &&
    ...
    {
        dlog.addText(Logger::TEAM, __FILE__ ": Bhv_BasicTackle. not necessary");
        return false;
    }
    return false;
}
