...
class IntentionReceive : public rcsc::SoccerIntention
{
  private:
  ...
  public:
    IntentionReceive(const rcsc::Vector2D &target_point, const double &dash_power, const double &buf,
                     const int max_step, const rcsc::GameTime &start_time);

    bool finished(const rcsc::PlayerAgent *agent);
    void set_stop(rcsc::PlayerAgent *agent);
