...
bool Bhv_SetPlay::execute(PlayerAgent *agent)
{
    ...
    if (wm.self().goalie())
    {
        if (wm.gameMode().type() != GameMode::BackPass_ && wm.gameMode().type() != GameMode::IndFreeKick_)
        {
...
void Bhv_SetPlay::doMove(PlayerAgent *agent)
{
    const WorldModel &wm = agent->world();
    ...
    /*if (wm.gameMode().side() != wm.ourSide())
        if (!can_go_to(-11, wm, Circle2D(wm.ball().pos(), 11.0),
       target_point)) target_point = get_avoid_circle_point(wm,
       target_point);*/
...
#include "body_pass.h"
void Bhv_SetPlay::doKick(PlayerAgent *agent)
{
    const WorldModel &wm = agent->world();
    ...
    //
    AngleDeg ball_place_angle = (wm.ball().pos().y > 0.0 ? -90.0 : 90.0);
    ...
    //
    {
        ...
        if (receiver && receiver->distFromBall() < 10.0 &&
            ...
            receiver->pos().absY() < ServerParam::i().pitchHalfWidth())
        {
            ...
            int ball_reach_step = ball_move_dist / 2.5;
            ...
            dlog.addText(Logger::TEAM, __FILE__ ":  kick to nearest teammate (%.1f %.1f) speed=%.2f", target_point.x,
                         target_point.y, ball_speed);
    ...
    if ((wm.ball().angleFromSelf() - wm.self().body()).abs() > 1.5)
    {
        ...
        dlog.addText(Logger::TEAM, __FILE__ ":  clear. turn to ball");
    ...
    //
    {
        ...
        Vector2D target_point(ServerParam::i().pitchHalfLength() - 2.0,
                              (ServerParam::i().pitchHalfWidth() - 5.0) *
                                  (1.0 - (wm.self().pos().x / ServerParam::i().pitchHalfLength())));
        if (wm.self().pos().y < 0.0)
        {
            target_point.y *= -1.0;
        }
        // enforce one step kick
        dlog.addText(Logger::TEAM, __FILE__ ": advance(2) to (%.1f, %.1f)", target_point.x, target_point.y);
        Body_KickOneStep(target_point, ServerParam::i().ballSpeedMax()).execute(agent);
    }
}

bool Bhv_SetPlay::doKickWait(PlayerAgent *agent)
{
    const WorldModel &wm = agent->world();
    ...
    if (real_set_play_count >= ServerParam::i().dropBallTime() - 5)
    {
        dlog.addText(Logger::TEAM,
                     __FILE__ ": (doKickWait) real set play count = %d > "
                              "drop_time-10, force kick mode",
                     real_set_play_count);
        return false;
    }
    ...
    if (Bhv_SetPlay::is_delaying_tactics_situation(agent))
    {
        ...
        dlog.addText(Logger::TEAM, __FILE__ ": (doKickWait) delaying");
    ...
    if (wm.teammatesFromBall().empty())
    {
        ...
        dlog.addText(Logger::TEAM, __FILE__ ": (doKickWait) no teammate");
    ...
    if (wm.setplayCount() <= 3)
    {
        ...
        dlog.addText(Logger::TEAM, __FILE__ ": (doKickWait) wait teammates");

        Body_TurnToBall().execute(agent);
        return true;
    }

    if (wm.setplayCount() >= 15 && wm.seeTime() == wm.time() && wm.self().stamina() > 8000)
    {
        dlog.addText(Logger::TEAM, __FILE__ ": (doKickWait) set play count = %d, force kick mode", wm.setplayCount());
        return false;
    }

    if (wm.seeTime() != wm.time() || wm.self().stamina() < 8000)
    {
        Body_TurnToBall().execute(agent);

        agent->debugClient().addMessage("KickIn:Wait%d", wm.setplayCount());
        dlog.addText(Logger::TEAM, __FILE__ ": (doKickWait) no see or recover");
        return true;
    }
...
double Bhv_SetPlay::get_set_play_dash_power(const PlayerAgent *agent)
{
    ...
    if (!wm.gameMode().isOurSetPlay(wm.ourSide()))
    {
        ...
        {
            if (wm.ball().pos().x < -30.0 && target_point.x < wm.ball().pos().x)
            {
            ...
            {
                rate =
                    0.9 * (wm.self().stamina() - ServerParam::i().recoverDecThrValue()) / ServerParam::i().staminaMax();
                rate = std::max(0.0, rate);
            }

            return (wm.self().playerType().staminaIncMax() * wm.self().recovery() * rate);
        }
    }
...
bool Bhv_SetPlay::can_go_to(const int count, const WorldModel &wm, const Circle2D &ball_circle,
                            const Vector2D &target_point)
{
    ...
    dlog.addText(Logger::TEAM, "%d: (can_go_to) check target=(%.2f %.2f) intersection=%d", count, target_point.x,
                 target_point.y, n_intersection);
    ...
    if (n_intersection == 1)
    {
        ...
        dlog.addText(Logger::TEAM, "%d: (can_go_to) intersection=1 angle_diff=%.1f", count,
...
Vector2D Bhv_SetPlay::get_avoid_circle_point(const WorldModel &wm, const Vector2D &target_point)
{
...
#ifdef DEBUG_PRINT
    dlog.addText(Logger::TEAM, __FILE__ ": (get_avoid_circle_point) first_target=(%.2f %.2f)", target_point.x,
                 target_point.y);
    dlog.addCircle(Logger::TEAM, wm.ball().pos(), avoid_radius, "#ffffff");
#endif

    if (can_go_to(-1, wm, ball_circle, target_point))
    {
        dlog.addText(Logger::TEAM, __FILE__ ": (get_avoid_circle_point) ok, first point");
        return target_point;
    }
...
#ifdef DEBUG_PRINT
    dlog.addText(Logger::TEAM,
                 __FILE__ ": (get_avoid_circle_point) "
                          "ball_target_angle=%.1f angle_step=%d",
    ...
    for (int i = 1; i < ANGLE_DIVS; ++i, a += angle_step, ++count)
    {
        ...
        Vector2D new_target = wm.ball().pos() + Vector2D::from_polar(avoid_radius + 1.0, angle);

        dlog.addText(Logger::TEAM, "%d: a=%d angle=%.1f (%.2f %.2f)", count, a, angle.degree(), new_target.x,
                     new_target.y);
...
bool Bhv_SetPlay::is_kicker(const PlayerAgent *agent)
{
    ...
    if (wm.gameMode().type() == GameMode::GoalieCatch_ && wm.gameMode().side() == wm.ourSide() && !wm.self().goalie())
    {
    ...
    dlog.addText(Logger::TEAM, __FILE__ ": (is_kicker) kicker_unum=%d second_kicker_unum=%d", kicker_unum,
                 second_kicker_unum);
    ...
    if (!kicker)
    {
        ...
        {
            dlog.addText(Logger::TEAM, __FILE__ ": (is_kicker) first kicker", kicker_unum, second_kicker_unum);
            return false;
        }
    ...
    if (kicker && second_kicker && (kicker->unum() == wm.self().unum() || second_kicker->unum() == wm.self().unum()))
    {
        if (std::sqrt(min_dist2) < std::sqrt(second_min_dist2) * 0.95)
        {
            dlog.addText(Logger::TEAM, __FILE__ ": (is_kicker) kicker->unum=%d  (1)", kicker->unum());
            return (kicker->unum() == wm.self().unum());
        }
        else if (kicker->distFromBall() < second_kicker->distFromBall() * 0.95)
        {
            dlog.addText(Logger::TEAM, __FILE__ ": (is_kicker) kicker->unum=%d  (2)", kicker->unum());
            return (kicker->unum() == wm.self().unum());
        }
        else if (second_kicker->distFromBall() < kicker->distFromBall() * 0.95)
        {
            dlog.addText(Logger::TEAM, __FILE__ ": (is_kicker) kicker->unum=%d  (3)", kicker->unum());
        ...
        {
            dlog.addText(Logger::TEAM, __FILE__ ": (is_kicker) other kicker", kicker->unum());
            return false;
        }
        else
        {
    ...
    dlog.addText(Logger::TEAM, __FILE__ ": (is_kicker) kicker->unum=%d", kicker->unum());
...
bool Bhv_SetPlay::is_delaying_tactics_situation(const PlayerAgent *agent)
{
    ...
    long cycle_thr = std::max(0, ServerParam::i().nrNormalHalfs() * (ServerParam::i().halfTime() * 10) - 500);
...
bool Bhv_SetPlay::GoToStaticBall(PlayerAgent *agent, const AngleDeg &ball_place_angle)
{
    ...
    if (angle_diff.abs() < dir_margin &&
        wm.ball().distFromSelf() < (wm.self().playerType().playerSize() + ServerParam::i().ballSize() + 0.08))
    {
        // already reach
        return false;
    }
    ...
    Vector2D sub_target = wm.ball().pos() + Vector2D::polar2vector(2.0, ball_place_angle + 180.0);
    ...
    if (angle_diff.abs() > dir_margin)
    {
        dlog.addText(Logger::TEAM, __FILE__ ": go to sub-target(%.1f, %.1f)", sub_target.x, sub_target.y);
    ...
    else
    {
        ...
        if ((wm.ball().angleFromSelf() - wm.self().body()).abs() > 1.5)
        {
            dlog.addText(Logger::TEAM, __FILE__ ": turn to ball");
            Body_TurnToBall().execute(agent);
        }
        // dash to ball
        else
