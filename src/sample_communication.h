...
class SampleCommunication : public Communication
{
  ...
  public:
    ...
    int currentSenderUnum() const
    {
        return M_current_sender_unum;
    }

    int nextSenderUnum() const
    {
        return M_next_sender_unum;
    }

  private:
    void updateCurrentSender(const rcsc::PlayerAgent *agent);

    void updatePlayerSendTime(const rcsc::WorldModel &wm, const rcsc::SideID side, const int unum);
