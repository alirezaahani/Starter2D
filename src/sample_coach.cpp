...
struct RealSpeedMaxCmp : public std::binary_function<const PlayerType *, const PlayerType *, bool>
{
...
*/
SampleCoach::SampleCoach() : CoachAgent()
{
    //
    ...
    // addSayMessageParser( SayMessageParser::Ptr( new
    // FreeMessageParser< 9 >( audio_memory ) ) );
    // addSayMessageParser( SayMessageParser::Ptr( new
    // FreeMessageParser< 8 >( audio_memory ) ) );
    // addSayMessageParser( SayMessageParser::Ptr( new
    // FreeMessageParser< 7 >( audio_memory ) ) );
    // addSayMessageParser( SayMessageParser::Ptr( new
    // FreeMessageParser< 6 >( audio_memory ) ) );
    // addSayMessageParser( SayMessageParser::Ptr( new
    // FreeMessageParser< 5 >( audio_memory ) ) );
    // addSayMessageParser( SayMessageParser::Ptr( new
    // FreeMessageParser< 4 >( audio_memory ) ) );
    // addSayMessageParser( SayMessageParser::Ptr( new
    // FreeMessageParser< 3 >( audio_memory ) ) );
    // addSayMessageParser( SayMessageParser::Ptr( new
    // FreeMessageParser< 2 >( audio_memory ) ) );
    // addSayMessageParser( SayMessageParser::Ptr( new
    // FreeMessageParser< 1 >( audio_memory ) ) );
 ...
 */
bool SampleCoach::initImpl(CmdLineParser &cmd_parser)
{
...
*/
void SampleCoach::actionImpl()
{
    debugClient().addMessage("Cycle=%ld", world().time().cycle());

    if (world().time().cycle() == 0 && config().useTeamGraphic() &&
...
*/
void SampleCoach::handleServerParam()
{
}
...
*/
void SampleCoach::handlePlayerParam()
{
}
...
*/
void SampleCoach::handlePlayerType()
{
}
...
*/
void SampleCoach::doSubstitute()
{
    static bool S_first_substituted = false;

    if (!S_first_substituted && world().time().cycle() == 0 && world().time().stopped() > 10)
    {
        doFirstSubstitute();
    ...
    if (world().time().cycle() > 0 && world().gameMode().type() != GameMode::PlayOn &&
...
*/
void SampleCoach::doFirstSubstitute()
{
    PlayerTypePtrCont candidates;

    std::fprintf(stderr, "id speed step inc  power  stam"
                         //" decay"
                         //" moment"
                         //" dprate"
                         "  karea"
    ...
    for (int id = 0; id < PlayerParam::i().playerTypes(); ++id)
    {
        ...
        if (id == Hetero_Default && PlayerParam::i().allowMultDefaultType())
        {
        ...
        std::fprintf(stderr,
                     ...
                     //"  %.3f  %.3f"
                     "\n",
                     id, param->realSpeedMax(), param->cyclesToReachMaxSpeed(), param->staminaIncMax(),
                     param->getDashPowerToKeepMaxSpeed(), param->getOneStepStaminaComsumption(),
                     // param->playerDecay(),
                     // param->inertiaMoment(),
                     // param->dashPowerRate(),
                     param->kickableArea()
    ...
    for (std::vector<int>::iterator unum = ordered_unum.begin(); unum != ordered_unum.end(); ++unum)
    {
        ...
        if (!p)
        {
            ...
            dlog.addText(Logger::TEAM,
                         __FILE__ ": teammate %d does not exist. "
                                  "skip first substitution.",
                         *unum);
            continue;
        }
...
*/
void SampleCoach::doSubstituteTiredPlayers()
{
    ...
    dlog.addText(Logger::TEAM, __FILE__ ": consider to substitute tired teammates.");
    ...
    for (std::vector<const GlobalPlayerObject *>::const_iterator t = world().teammates().begin(),
    ...
    {
        dlog.addText(Logger::TEAM, __FILE__ ": no tired teammates.");
        return;
    }
    ...
    for (std::vector<int>::const_iterator id = world().availablePlayerTypeId().begin(),
    ...
    for (std::vector<int>::iterator unum = tired_teammate_unum.begin(); unum != tired_teammate_unum.end(); ++unum)
    {
        int type = getFastestType(candidates);
        if (type != Hetero_Unknown)
        {
 ...
 */
void SampleCoach::substituteTo(const int unum, const int type)
{
    if (world().time().cycle() > 0 && world().ourSubstituteCount() >= PlayerParam::i().subsMax())
    {
        std::cerr << "***Warning*** " << config().teamName() << " coach: over the substitution max."
                  << " cannot change the player " << unum << " to type " << type << std::endl;
        return;
    }

    std::vector<int>::const_iterator it =
        std::find(world().availablePlayerTypeId().begin(), world().availablePlayerTypeId().end(), type);
    if (it == world().availablePlayerTypeId().end())
    {
        std::cerr << "***ERROR*** " << config().teamName() << " coach: "
                  << " cannot change the player " << unum << " to type " << type << std::endl;
        return;
    }
    ...
    std::cout << config().teamName() << " coach: "
              << "change player " << unum << " to type " << type << std::endl;
}
 ...
 */
int SampleCoach::getFastestType(PlayerTypePtrCont &candidates)
{
    if (candidates.empty())
    {
    ...
    // sort by max speed
    std::sort(candidates.begin(), candidates.end(), RealSpeedMaxCmp());
    ...
    int min_cycle = 100;
    for (PlayerTypePtrCont::iterator it = candidates.begin(); it != candidates.end(); ++it)
    {
        ...
        if ((*it)->cyclesToReachMaxSpeed() == min_cycle)
        {
            if ((*it)->getOneStepStaminaComsumption() < (*best_type)->getOneStepStaminaComsumption())
            {
                best_type = it;
                max_speed = (*best_type)->realSpeedMax();
            }
...
*/
void SampleCoach::sayPlayerTypes()
{
    /*
      format:
      "(player_types (1 0) (2 1) (3 2) (4 3) (5 4) (6 5) (7 6) (8 -1)
      (9 0) (10 1) (11 2))"
      ->
    ...
    */
    ...
    {
        char buf[8];
        snprintf(buf, 8, "(%d %d)", unum, M_opponent_player_types[unum - 1]);
        msg += buf;
    }
    ...
    std::cout << config().teamName() << " coach: " << world().time() << " sent freeform " << msg << std::endl;
}
...
*/
void SampleCoach::sendTeamGraphic()
{
    int count = 0;
    for (TeamGraphic::Map::const_reverse_iterator tile = M_team_graphic.tiles().rbegin();
         tile != M_team_graphic.tiles().rend(); ++tile)
    {
        if (teamGraphicOKSet().find(tile->first) == teamGraphicOKSet().end())
        {
            if (!doTeamGraphic(tile->first.first, tile->first.second, M_team_graphic))
            {
                break;
            }
            ++count;
    ...
    if (count > 0)
    {
        std::cout << config().teamName() << " coach: " << world().time() << " send team_graphic " << count << " tiles"
                  << std::endl;
    }
}
