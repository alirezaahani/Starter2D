...
int main(int argc, char **argv)
{
    ...
    if (sigaction(SIGINT, &sig_action, NULL) != 0 || sigaction(SIGTERM, &sig_action, NULL) != 0 ||
    ...
    {
        std::cerr << __FILE__ << ": " << __LINE__ << ": could not set signal handler: " << std::strerror(errno)
                  << std::endl;
        std::exit(EXIT_FAILURE);
    }
     ...
     */
    std::cout << "*****************************************************************\n"
              << " This program is modified by Robot2D\n"
              << " Copyright 2022. Alireza Ahani.\n"
              << " Copyright 2022. Alireza Hasankhani.\n"
              << " Copyright 2022. Matin Noori.\n"
              << " All rights reserved.\n"
              << "*****************************************************************\n"
              << std::flush;
    ...
    std::cout << "***************************************************"
                 "**************\n"
              << " This program is based on agent2d created by "
                 "Hidehisa Akiyama.\n"
              << " Copyright 2006 - 2011. Hidehisa Akiyama and "
                 "Hiroki Shimora.\n"
              << " All rights reserved.\n"
              << "***************************************************"
                 "**************\n"
