...
SamplePlayer::SamplePlayer() : PlayerAgent(), M_communication()
{
    ...
    //
    // M_action_generators.push_back( ActionGenerator::Ptr( new
    // PassGenerator() ) );
    ...
    if (this->world().gameMode().type() == GameMode::PlayOn)
    {
        if (this->world().self().goalie())
        {
            ...
            static const Rect2D our_penalty(
                Vector2D(-ServerParam::i().pitchHalfLength(), -ServerParam::i().penaltyAreaHalfWidth() + 1.0),
                Size2D(ServerParam::i().penaltyAreaLength() - 1.0, ServerParam::i().penaltyAreaWidth() - 2.0));
        ...
        else
        {
            ...
            if (this->world().existKickableTeammate() &&
                this->world().teammatesFromBall().front()->distFromBall() < this->world().ball().distFromSelf())
            {
                kickable = false;
            }
...
void SamplePlayer::handleActionEnd()
{
    ...
    {
        // top,lower
        debugClient().addLine(Vector2D(world().ourDefenseLineX(), world().self().pos().y - 2.0),
                              Vector2D(world().ourDefenseLineX(), world().self().pos().y + 2.0));

        //
        debugClient().addLine(Vector2D(world().offsideLineX(), world().self().pos().y - 15.0),
    ...
    //
    dlog.addText(Logger::WORLD, "WM: BALL pos=(%lf, %lf), vel=(%lf, %lf, r=%lf, ang=%lf)", world().ball().pos().x,
                 world().ball().pos().y, world().ball().vel().x, world().ball().vel().y, world().ball().vel().r(),
                 world().ball().vel().th().degree());

    dlog.addText(Logger::WORLD, "WM: SELF move=(%lf, %lf, r=%lf, th=%lf)", world().self().lastMove().x,
                 world().self().lastMove().y, world().self().lastMove().r(), world().self().lastMove().th().degree());

    Vector2D diff = world().ball().rpos() - world().ball().rposPrev();
    dlog.addText(Logger::WORLD, "WM: BALL rpos=(%lf %lf) prev_rpos=(%lf %lf) diff=(%lf %lf)", world().ball().rpos().x,
                 world().ball().rpos().y, world().ball().rposPrev().x, world().ball().rposPrev().y, diff.x, diff.y);

    Vector2D ball_move = diff + world().self().lastMove();
    Vector2D diff_vel = ball_move * ServerParam::i().ballDecay();
    dlog.addText(Logger::WORLD, "---> ball_move=(%lf %lf) vel=(%lf, %lf, r=%lf, th=%lf)", ball_move.x, ball_move.y,
                 diff_vel.x, diff_vel.y, diff_vel.r(), diff_vel.th().degree());
}
...
bool SamplePlayer::doPreprocess()
{
    ...
    if (wm.self().isFrozen())
    {
        dlog.addText(Logger::TEAM, __FILE__ ": tackle wait. expires= %d", wm.self().tackleExpires());
        // face neck to ball
        return true;
    }
    ...
    if (wm.gameMode().type() == GameMode::BeforeKickOff || wm.gameMode().type() == GameMode::AfterGoal_)
    {
        ...
        KickOffPosition[1] = Vector2D(-49, 0);
        KickOffPosition[2] = Vector2D(-12.5, -5);
        KickOffPosition[3] = Vector2D(-12.5, 5);
        KickOffPosition[4] = Vector2D(-11.6, -15.8);
        KickOffPosition[5] = Vector2D(-11.6, 15.8);
        KickOffPosition[6] = Vector2D(-6.7, -1.9);
        KickOffPosition[7] = Vector2D(-10, -10.8);
        KickOffPosition[8] = Vector2D(-10, 10.8);
        KickOffPosition[9] = Vector2D(-0.5, -23.8);
        KickOffPosition[10] = Vector2D(-0.5, 23.8);
        KickOffPosition[11] = Vector2D(-0.39, 0);
    ...
    if (wm.ball().posCount() > count_thr ||
        (wm.gameMode().type() != GameMode::PlayOn && wm.ball().seenPosCount() > count_thr + 10))
    {
...
bool SamplePlayer::doForceKick()
{
    ...
    if (wm.gameMode().type() == GameMode::PlayOn && !wm.self().goalie() && wm.self().isKickable() &&
...
bool SamplePlayer::doHeardPassReceive()
{
    ...
    if (wm.audioMemory().passTime() != wm.time() || wm.audioMemory().pass().empty() ||
    ...
    dlog.addText(Logger::TEAM,
                 __FILE__ ":  (doHeardPassReceive) heard_pos(%.2f "
                          "%.2f) intercept_pos(%.2f %.2f)",
                 heard_pos.x, heard_pos.y, intercept_pos.x, intercept_pos.y);

    if (!wm.existKickableTeammate() && wm.ball().posCount() <= 1 && wm.ball().velCount() <= 1 && self_min < 20
        //&& intercept_pos.dist( heard_pos ) < 3.0 ) //5.0 )
    )
    {
        dlog.addText(Logger::TEAM, __FILE__ ": (doHeardPassReceive) intercept cycle=%d. intercept", self_min);
    ...
    {
        dlog.addText(Logger::TEAM,
                     __FILE__ ": (doHeardPassReceive) intercept "
                              "cycle=%d. go to receive point",
                     self_min);
        this->debugClient().setTarget(heard_pos);
        this->debugClient().addMessage("Comm:Receive:GoTo");
        Body_GoToPoint(heard_pos, 0.5, ServerParam::i().maxDashPower()).execute(this);
    }

    this->setIntention(new IntentionReceive(heard_pos, ServerParam::i().maxDashPower(), 0.9, 5, wm.time()));
