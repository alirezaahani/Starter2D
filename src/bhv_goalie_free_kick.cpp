...
/*!
  execute action
*/
bool Bhv_GoalieFreeKick::execute(rcsc::PlayerAgent *agent)
{
    static bool s_first_move = false;
    static bool s_second_move = false;
    static int s_second_wait_count = 0;

    rcsc::dlog.addText(rcsc::Logger::TEAM, __FILE__ ": Bhf_GoalieFreeKick");
    if (agent->world().gameMode().type() != rcsc::GameMode::GoalieCatch_ ||
        agent->world().gameMode().side() != agent->world().ourSide() || !agent->world().self().isKickable())
    {
        rcsc::dlog.addText(rcsc::Logger::TEAM, __FILE__ ": Bhv_GoalieFreeKick. Not a goalie catch mode");
    ...
    */
    ...
    if (!s_first_move)
    {
        // rcsc::Vector2D move_target(
        // rcsc::ServerParam::i().ourPenaltyAreaLine() - 0.8, 0.0 );
    ...
    if (time_diff < 50 || agent->world().setplayCount() < 3 ||
...
*/
rcsc::Vector2D Bhv_GoalieFreeKick::getKickPoint(const rcsc::PlayerAgent *agent)
{
    ...
    const rcsc::PlayerPtrCont::const_iterator opps_end = agent->world().opponentsFromSelf().end();
    for (rcsc::PlayerPtrCont::const_iterator o = agent->world().opponentsFromSelf().begin(); o != opps_end; ++o)
    {
        for (std::vector<std::pair<rcsc::Vector2D, double>>::iterator it = candidates.begin(); it != candidates.end();
             ++it)
        {
    ...
    for (std::vector<std::pair<rcsc::Vector2D, double>>::iterator it = candidates.begin(); it != candidates.end(); ++it)
    {
...
*/
#include "body_pass.h"
void Bhv_GoalieFreeKick::doKick(rcsc::PlayerAgent *agent)
{
...
*/
void Bhv_GoalieFreeKick::doWait(rcsc::PlayerAgent *agent)
{
    const rcsc::WorldModel &wm = agent->world();
    rcsc::Vector2D face_target(0.0, 0.0);

    if (wm.self().pos().x > rcsc::ServerParam::i().ourPenaltyAreaLineX() - rcsc::ServerParam::i().ballSize() -
                                wm.self().playerType().playerSize() - 0.5)
    {
